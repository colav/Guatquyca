...
test.describe("'Pedagogía del oprimido...' article information is complete", () => {
  test("'Pedagogía del oprimido...' article information is complete", async ({
    page,
  }) => {
    ...
    await test.step("Check that the listed authors match the expected names, ignoring accents and sorting alphabetically", async () => {
      const expectedAuthors = ["jose oliverio tovar bohorquez"].sort();
    ...
    await test.step("Ensure the publication date is correctly displayed as 'Publicado: 2015' in the article modal", async () => {
      await expect(modal.getByText("Publicado: 2015")).toBeVisible();
    });
    ...
    "Check that the OpenAlex citation count is displayed and that at least one citation element exists, with a value above the threshold", async () => {
      ...
      // Allow a 20% error margin below the expected citation count (5)
      const threshold = 5 * 0.8;
      expect(citationCount).toBeGreaterThanOrEqual(threshold);
    });

    // Verify DOI link
    await test.step("Verify that the DOI link (https://doi.org/10.5294/pecu.2015.18.1.6) is present in the modal", async () => {
      await expect(
        modal
    ...
    // Verify Scienti IDs
    const scientiIDs = ["0000497967-11"];
    await test.step(
    ...
    await test.step("Check that the correct journal name ('Pensamiento Y Cultura') and ISSN ('0123-0999') are displayed", async () => {
      await expect(
