...
export default function RenderedExternalIDs(external_ids = []) {
  ...
  return external_ids.flatMap((item, index) => {
    // Use a combination of id and source as the unique key
    const uniqueKey = `${item.id}-${item.source}`;
    ...
    if (item.source !== "minciencias" && item.source !== "scienti") {
      ...
      return [
        {
          key: `ext-id-${index}-1`,
          label: `${getSourceLabel(item.source)} ID`,
          children: (
            <Tooltip title={`Fuente: ${item.provenance}`}>{item.id}</Tooltip>
          ),
        },
      ];
    } else {
