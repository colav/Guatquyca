...
export const filterMenuMaker = (data) => {
  ...
  const filterComponents = {
    ...
    cities: {
      ...
      label: (
        <>
          <EnvironmentOutlined style={{ fontSize: "16px" }} /> Ciudad
        </>
      ),
    },
    states: {
      ...
      label: (
        <>
          <EnvironmentOutlined style={{ fontSize: "16px" }} /> Departamento /
          Estado
        </>
      ),
    },
    ...
    .map((key) => {
      ...
      if (mapping) {
        ...
        return {
          key: key,
          label: label,
          children: <Component key={key} data={data[key]} filterType={key} />,
        };
      }
      return null;
    })
